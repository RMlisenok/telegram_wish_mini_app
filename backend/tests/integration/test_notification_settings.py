...
class TestScenario9NotificationSettings:
    """Сценарий 9: Настройка уведомлений пользователя"""
    ...
    async def test_negative_update_invalid_settings(
        ...
        API игнорирует неизвестные поля, поэтому проверяем что ответ 200,
        но неизвестные поля не сохраняются
        """
        ...
        # API возвращает 200, игнорируя неизвестные поля
        assert response.status_code == 200
    ...
    async def test_negative_update_wrong_type(
        ...
        """
        Негативный сценарий: Обновление с неправильным типом данных
    ...
    async def test_negative_update_without_auth(
        ...
        """
        Негативный сценарий: Обновление настроек без авторизации
        """
