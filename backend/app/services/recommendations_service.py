    ...
    async def generate_and_send_via_bot(
        ...
        async with db_factory() as session:
            try:
                ...
                if not target_user:
                    await bot.send_message(
                        requester_id, "❌ Пользователь не найден."
                    )
                    return

                stmt = select(UserForm).where(
                    UserForm.user_id == target_user.id
                )
                result = await session.execute(stmt)
                user_forms = result.scalars().all()
                ...
                gifts = await RecommendationService.get_recommendations(
                ...
                await bot.send_message(
                    requester_id,
                    "Сервис рекомендаций временно недоступен. Попробуйте позже"
                )
