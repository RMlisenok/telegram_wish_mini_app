...
class WishReservationRepository:
    ...
    async def get(
        ...
        query = select(WishReservation).where(
            WishReservation.id == wish_reservation_id
        )
    ...
    async def get_user_reservations(
        ...
        query = select(WishReservation).where(
            WishReservation.reserved_by_id == reserved_id
        ).limit(limit).options(
            selectinload(WishReservation.wish_wishlist).selectinload(WishWishlist.wish),
            selectinload(WishReservation.wish_wishlist).selectinload(WishWishlist.wishlist)
        )
        result = await self.session.execute(query)
    ...
    async def check_wish_reservation(
        ...
        query = select(WishReservation).where(
            WishReservation.wish_wishlist_id == wish_wihslist_id)
        result = await self.session.execute(query)
